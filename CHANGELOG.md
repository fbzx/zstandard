...
- A stream that pledged exactly one block of content, which is every pledged stream of 128 KiB or less, spent three bytes more than the one-shot encoder on the same input. The block size is capped at the pledge, so the only block filled on the last pledged byte and went out unflagged, and `finish` then closed the frame with an empty last block. C sets its buffer target one byte past the block size when the pledge equals it (`ZSTD_CCtx_init_compressStream2`), so its only block waits for the end and carries the flag itself; `StreamingEncoder` and `io::Writer` now do the same, and a pledged single-block stream is byte-identical to `encode_all_with_options` and to upstream's streaming encoder under the same pledge. Frames were valid either way; the comment on `finish` that attributed the empty block to the format was wrong for this case.
