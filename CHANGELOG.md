...
- A stream that pledged one block of content or less, which at default parameters is any pledge of 128 KiB or less, spent three bytes more than the one-shot encoder on the same input. The block size is capped at the pledge, so the only block filled on the last pledged byte and went out unflagged, and `finish` then closed the frame with an empty last block. `StreamingEncoder` and `io::Writer` now hold that block back for `finish` to flag, as C does with `inBuffTarget = blockSizeMax + (blockSizeMax == pledgedSrcSize)`, so the frame is byte-identical to `encode_all_with_options` and to upstream's streaming encoder under the same pledge. Frames were valid either way. Content that is an exact multiple of the block size and longer than one block still carries the empty last block, which is upstream's behavior too.
...
[0.1.8]: https://github.com/stephenberry/zstandard/releases/tag/v0.1.8
[0.1.7]: https://github.com/stephenberry/zstandard/releases/tag/v0.1.7
[0.1.6]: https://github.com/stephenberry/zstandard/releases/tag/v0.1.6
[0.1.5]: https://github.com/stephenberry/zstandard/releases/tag/v0.1.5
